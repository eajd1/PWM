#Current Version made 12/nov/2019
#First Version late 2018
#Current Version 14/dec/2019
#By Ethan Denis
...
class PWM:
    ...
    def __init__(self, master):
        ...
        self.display = tk.Text(self.output_frame, width = 100, height = 30, yscrollcommand = self.display_scroll.set)
        ...
        self.file_entry = tk.Entry(self.input_frame, width = 40)
        ...
        self.file_display = tk.Text(self.output_frame, width = 50, height = 30, yscrollcommand = self.file_display_scroll.set)
        self.file_display.grid(row = 0, column = 2)
        ...
        self.forward_folder = tk.Button(self.input_frame, text = 'Forward', width = 10,
                                        command = self.go_forward, cursor = 'hand2',
                                        state = 'disabled')
        self.forward_folder.grid(row = 1, column = 14)

        self.back_folder = tk.Button(self.input_frame, text = 'Backward', width = 10,
                                     command = self.go_back, cursor = 'hand2')
        self.back_folder.grid(row = 2, column = 14)
        ...
        self.clear_forward = tk.Button(self.input_frame, text = 'Clear Forward', width = 10,
                                       command = self.clear_forward_array, cursor = 'hand2',)
        self.clear_forward.grid(row = 2, column = 13)
    ...
    def encrypt(self):
        ...
        newmsg = encrypting(msg, password, pin)
    ...
    def decrypt(self):
        ...
        password = self.pass_entry.get()
        pin = self.pin_entry.get()
        newmsg = decrypting(msg, password, pin)
        self.display.delete('1.0',tk.END)
        self.display.insert(tk.END, newmsg)
        root.focus_set()
    ...
    def list_default_files(self, files):
        ...
        for i in range(0, len(files)):
            if os.path.isdir(files[i].replace('\n','')):
                self.file_display.insert(str(float(i)), u'\u2220 '+files[i])
                total += 1
        for i in range(0, len(files)):
            if '.txt' in files[i] or '.' not in files[i] and not os.path.isdir(files[i].replace('\n','')):
                self.file_display.insert(str(float(i+total)), files[i])
    ...
    def create_dir(self):
        ...
        self.list_files()
        self.forward_folder.config(state = 'normal')
        self.previous_path.append(self.path_make_entry.get())
    ...
    def remove_dir_event(self, event):
        ...
        self.list_files_event(event)
        self.confirm_window.destroy()
        self.previous_path.remove(self.path_make_entry.get())
        self.forward_folder.config(state = 'disabled')
    ...
    def go_forward(self):
        self.master.focus_set()
        path = self.path_entry.get()
        self.path_var.set(path+self.previous_path[len(self.previous_path)-1])
        del self.previous_path[len(self.previous_path)-1]
        if not self.previous_path:
            self.forward_folder.config(state = 'disabled')
        self.list_files()
        self.back_folder.config(state = 'normal')
    ...
    def go_back(self):
        ...
        for i in range(0, len(path)):
            if path[i] == '\\' and i > slash:
                slash = i
        self.previous_path.append(path[slash:])
        path = path[0:slash]
        if path == 'C:':
